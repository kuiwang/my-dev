...
Object.extend(String.prototype, (function() {
    ...
    function gsub(pattern, replacement) {
        ...
        while (source.length > 0) {
            match = source.match(pattern) if (match && match[0].length > 0) {
...
(function() {
    ...
    function each(iterator, context) {
        for (var i = 0,
        length = this.length >>> 0; i < length; i++) {
    ...
    function concat(_) {
        ...
        length = items.length; i < length; i++) {
            ...
            if (Object.isArray(item) && !('callee' in item)) {
                for (var j = 0,
                arrayLength = item.length; j < arrayLength; j++) {
    ...
    function map(iterator) {
        ...
        for (var i = 0,
        length = object.length >>> 0; i < length; i++) {
            if (i in object) {
    ...
    setRequestHeaders: function() {
       ...
       */
        ...
        if (typeof this.options.requestHeaders == 'object') {
            ...
            if (Object.isFunction(extras.push)) for (var i = 0,
            length = extras.length; i < length; i += 2) headers[extras[i]] = extras[i + 1];
...
(function(GLOBAL) {
    ...
    function $(element) {
        if (arguments.length > 1) {
            for (var i = 0,
            elements = [], length = arguments.length; i < length; i++) elements.push($(arguments[i]));
            return elements;
        }
    ...
    function update(element, content) {
        ...
        if (ANY_INNERHTML_BUGGY) {
            if (tagName in INSERTION_TRANSLATIONS.tags) {
                ...
                for (var i = 0,
                node; node = nodes[i]; i++) element.appendChild(node);
    ...
    function insertContentAt(element, content, position) {
        ...
        for (var i = 0,
        node; node = childNodes[i]; i++) method(element, node);
    ...
    function down(element, expression, index) {
        ...
        if (Object.isNumber(expression)) index = expression,
        expression = '*';
    ...
    for (var i = 0,
    attr; attr = CAMEL_CASED_ATTRIBUTE_NAMES[i]; i++) {
    ...
    function addMethods(methods) {
        ...
        if (!tagName) {
            ...
            if (Object.isArray(tagName)) {
                for (var i = 0,
                tag; tag = tagName[i]; i++) addMethodsToTagName(tag, methods);
            } else {
...
Form.Methods = {
    ...
    getInputs: function(form, typeName, name) {
        ...
        for (var i = 0,
        matchingInputs = [], length = inputs.length; i < length; i++) {
            var input = inputs[i];
...
Form.Element.Serializers = (function() {
    ...
    function select(element, value) {
        ...
        for (var i = 0,
        length = element.length; i < length; i++) {

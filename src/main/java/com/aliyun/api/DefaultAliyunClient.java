...
public class DefaultAliyunClient implements AliyunClient {
    ...
    public DefaultAliyunClient(String serverUrl, String accessKeyId, String accessKeySecret,
            String format, int connectTimeout, int readTimeout, ExecutorService executorService) {
    ...
    public <T extends AliyunResponse> Map<String, Object> doPost(AliyunRequest<T> request)
        ...
        try {
            ...
            if (request instanceof AliyunUploadRequest) {
                ...
                rsp = AliyunWebUtils.doPost(url, null, fileParams, Constants.CHARSET_UTF8,
                        connectTimeout, readTimeout, request.getHeaderMap());
            } else {
                rsp = AliyunWebUtils.doPost(url, null, Constants.CHARSET_UTF8, connectTimeout,
                        readTimeout, request.getHeaderMap());
            }
        } catch (IOException e) {
            throw new ApiException(e);
        }
    ...
    private String paramsToQueryString(Map<String, String> params)
        ...
        for (Entry<String, String> p : params.entrySet()) {
            ...
            if (val != null) {
                paramString.append("=")
                        .append(URLEncoder.encode(val, AliyunConstants.CHARSET_UTF8));
            }

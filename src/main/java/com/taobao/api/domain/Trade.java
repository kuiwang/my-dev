...
public class Trade extends TaobaoObject {
    ...
    private static final long serialVersionUID = 3614334152365624795L;
    ...
    /**
     * 交易中剩余的确认收货金额（这个金额会随着子订单确认收货而不断减少，交易成功后会变为零）。精确到2位小数;单位:元。如:200.07，表示
     * :200 元7分
     */
     ...
     * REJECTED_BY_COMPANY, 接单超时 RECIEVE_TIMEOUT, 揽收成功 TAKEN_IN_SUCCESS,
     * 揽收失败 TAKEN_IN_FAILED, 揽收超时 TAKEN_TIMEOUT, 签收成功 SIGN_IN, 签收失败
     * REJECTED_BY_OTHER_SIDE, 订单等待发送给物流公司 WAITING_TO_BE_SENT, 用户取消物流订单
     * CANCELED
     */
    @ApiField("cod_status")
     ...
     * 比如：广东广州：广州属于一个直辖市是放在的receiver_city的字段里面；而河南济源：济源属于省直辖县级行政区划分，是区级别的，
     * 放在了receiver_district里面 <br>
    ...
    /**
     * 创建交易时的物流方式（交易完成前，物流方式有可能改变，但系统里的这个字段一直不变）。可选值：free(卖家包邮),post(平邮),
     * express(快递),ems(EMS),virtual(虚拟发货)，25(次日必达)，26(预约配送)。
     */
     ...
     * external_trade(统一外部交易) o2o_offlinetrade（O2O交易） step (万人团)
     * nopaid(无付款订单) pre_auth_type(预授权0元购机交易)
     */
    @ApiField("type")
    private String type;

...
import com.taobao.api.response.SpContentUpdateResponse;
 ...
 */
public class SpContentUpdateRequest implements TaobaoRequest<SpContentUpdateResponse> {
     ...
     * 它是U站首页或淘宝官网搜索到内容之后用户点击进入的跳转页面。（如果站点没有单个内容的detail页面，也
     * 
     * 可以直接填写站点首页）； items(String，宝贝必填)： 内容关联的商品，数值为商品的detail链接地址。此参数只有在type
     * = 1 （即内容类型为宝贝）的时候才有效，宝贝只能并且必须
     * 
     * 关联一个商品；title(String，宝贝和心得必填): 内容标题，数值为文本内容，此参数只有在type =
     * 1或3（即内容类型为心得或宝贝）的时候才有效，且标题的长度限制为(0,30](单位是字
     * 
     * 符，不区分中英文)，标题中不能含有非法内容，不能含有恶意脚本。 comments (String，宝贝必填)：
     * 内容的推荐理由，数值为文本内容，此参数只有在type =1 （即内
     * 
     * 容类型为宝贝）的时候才有效，且推荐理由的长度限制为[0,140](单位是字符，不区分中英文)，推荐理由中不能含有非法内容，
     * 不能含有恶意脚本。 content(String，心得必填)：内
     * 
     * 容的心得，数值为文本内容（html形式），此参数只有在type =
     * 3（即内容类型为心得）的时候才有效，且心得长度限制为[100,20000] (单位是字符，不区分中英文), 心得中不能有外链，
     * 
     * 不能有恶意脚本；心得中包含的商品链接系统自自动提取并保存起来;多个商品链接以空格隔开(如

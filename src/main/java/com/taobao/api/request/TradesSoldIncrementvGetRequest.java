...
public class TradesSoldIncrementvGetRequest
        implements TaobaoRequest<TradesSoldIncrementvGetResponse> {

    /**
     * 查询入库结束时间，必须大于入库开始时间(修改时间跨度不能大于一天)，格式:yyyy-MM-dd HH:mm:ss。
    ...
    /**
     * 需要返回的字段。目前支持有： 1.Trade中可以指定返回的fields:seller_nick, buyer_nick, title,
     * type, created, tid, seller_rate, buyer_rate, status, payment,
     * discount_fee, adjust_fee, post_fee, total_fee, pay_time, end_time,
     * modified, consign_time, buyer_obtain_point_fee, point_fee,
     * real_point_fee, received_payment,pic_path, num_iid, num, price,
     * cod_fee, cod_status, shipping_type, receiver_name, receiver_state,
     * receiver_city, receiver_district, receiver_address, receiver_zip,
     * receiver_mobile,
     * receiver_phone,alipay_id,alipay_no,is_lgtype,is_force_wlb
     * ,is_brand_sale
     * ,has_buyer_message,credit_card_fee,step_trade_status,step_paid_fee
     * ,mark_desc，is_daixiao,is_part_consign 2.Order中可以指定返回fields：
     ...
     * orders.adjust_fee, orders.sku_properties_name,
     * orders.item_meal_name, orders.buyer_rate, orders.seller_rate,
     * orders.outer_iid, orders.outer_sku_id, orders.refund_id,
     * orders.seller_type，orders.end_time,
     * orders.order_from,orders.consign_time
     * ,orders.shipping_type,orders.logistics_company
     * ,orders.invoice_no，orders.is_daixiao 3.fields：orders（返回Order的所有内容）
     * 4.f
     * ields:service_orders(返回service_order中所有内容),,orders.is_www(是否是www订单
     * ）,orders.store_code（仓库地址）
     */
     ...
     * external_trade(统一外部交易) instant_trade (即时到账) b2c_cod(大商家货到付款)
     * hotel_trade(酒店类型交易) super_market_trade(商超交易),
     * super_market_cod_trade(商超货到付款交易) taohua(桃花网交易类型） waimai(外卖交易类型）
     * o2o_offlinetrade（O2O交易） nopaid（无付款订单） eticket(电子凭证) tmall_i18n（天猫国际）

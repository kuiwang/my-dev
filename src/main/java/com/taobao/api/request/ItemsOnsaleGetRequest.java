...
public class ItemsOnsaleGetRequest implements TaobaoRequest<ItemsOnsaleGetResponse> {
     ...
     * approve_status,num_iid,title,nick,type,
     * cid,pic_url,num,props,valid_thru
     * ,list_time,price,has_discount,has_invoice
     * ,has_warranty,has_showcase,modified
     * ,delist_time,postage_id,seller_cids,outer_id；字段之间用“,”分隔。
     ...
     * ，column可选值:list_time(上架时间),delist_time(下架时间),
     * num(商品数量)，modified(最近修改时间
     * )，sold_quantity（商品销量）,;默认上架时间降序(即最新上架排在前面)。如按照上架时间降序排序方式为list_time
     * :desc
     */
    private String orderBy;

    /**
     * 页码。取值范围:大于零的整数。默认值为1,即默认返回第一页数据。用此接口获取数据时，当翻页获取的条数（page_no*page_size
     * ）超过10万,为了保护后台搜索引擎，接口将报错。所以请大家尽可能的细化自己的搜索条件，例如根据修改时间分段获取商品
     */
    private Long pageNo;

    /**
     * 每页条数。取值范围:大于零的整数;最大值：200；默认值：40。用此接口获取数据时，当翻页获取的条数（page_no*page_size
     * ）超过2万,为了保护后台搜索引擎，接口将报错。所以请大家尽可能的细化自己的搜索条件，例如根据修改时间分段获取商品
     */

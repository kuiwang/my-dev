...
public class FenxiaoOrdersGetRequest implements TaobaoRequest<FenxiaoOrdersGetResponse> {
     ...
     * 多个字段用","分隔。
     * 
     * fields 如果为空：返回所有采购单对象(purchase_orders)字段。
     * 如果不为空：返回指定采购单对象(purchase_orders)字段。
     * 
     * 例1： sub_purchase_orders.tc_order_id 表示只返回tc_order_id 例2：
    ...
    /**
     * 交易状态，不传默认查询所有采购单根据身份选择自身状态可选值:<br>
     * *供应商：<br>

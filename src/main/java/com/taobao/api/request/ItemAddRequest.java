...
public class ItemAddRequest implements TaobaoUploadRequest<ItemAddResponse> {
     ...
     * ，默认是5，即0.5%。注意此时该字段值依旧必须是>0的整数，最高值不超过500，即50%
     */
     ...
     * 基础色只支持以下14种颜色：28335//绿色 28338//蓝色 90554//桔色 28324//黄色 28341//黑色
     * 28320//白色 28326//红色 28329//紫色 3232480//粉红色 107121//透明 132069//褐色
     * 28332//浅灰色 3232478//深灰色 130164//花色
     */
    ...
    /**
     * 全球购商品采购地（库存类型）， 有两种库存类型：现货和代购 参数值为1时代表现货，值为2时代表代购。注意：使用时请与
     * 全球购商品采购地（地区/国家）配合使用
     */
     ...
     * 否则设置true后会返回错误码
     * :isv.invalid-permission:add-xinpin。同时只有一口价全新的宝贝才能设置为新品
     ...
     * 该值支持两种格式的设置：格式1：bulk:
     * 3,单位为立方米(m3),表示直接设置为商品的体积。格式2：length:10;breadth:10;height:10，单位为米（m）
     */
    ...
    /**
     * 自定义销售周期的小时数。拍卖宝贝可以自定义销售周期，这里指定销售周期的小时数。注意，该参数只作为输入参数，不能通过taobao.item
     * .get接口获取。<br />
     * 支持最大值为：48<br />
     * 支持最小值为：1
     */
    ...
    /**
     * 宝贝所属的运费模板ID。取值范围：整数且必须是该卖家的运费模板的ID（可通过taobao.delivery.template.
     * get获得当前会话用户的所有邮费模板）
     */
     ...
     * 注:post_fee,express_fee,ems_fee需要一起填写
     */
    ...
    /**
     * 商品卖点信息，最长150个字符。天猫商家和集市卖家都可用。<br />
     * 支持最大长度为：150<br />
     * 支持的最大列表长度为：150
     */
     ...
     * itempropvalues
     * .get取得vid。格式:一个SKU的pid1:vid;pid2:vid,多个sku之间属性用逗号分隔。该字段内的销售属性
     * （自定义的除外）也需要在props字段填写。sku的销售属性需要一同选取，如:颜色，尺寸。如果新增商品包含了sku，则此字段一定要传入。
     * 这个字段的长度要控制在512个字节以内。 如果有自定义销售属性，则格式为pid:vid;pid2:vid2;$pText:vText ,
     * 其中$pText
    ...
    /**
     * 宝贝标题。不能超过30字符，受违禁词控制。天猫图书管控类目最大允许120字符；<br />
     * 支持最大长度为：120<br />
     * 支持的最大列表长度为：120
     */
     ...
     * 拍卖商品发布时需要附加拍卖商品信息
     * ：拍卖类型(paimai_info.mode，拍卖类型包括三种：增价拍[1]，荷兰拍[2]以及降价拍[3
     * ])，商品数量(num)，起拍价(
     * price)，价格幅度(increament)，保证金(paimai_info.deposit)。另外拍卖商品支持自定义销售周期
     * ，通过paimai_info
     * .valid_hour和paimai_info.valid_minute来指定。对于降价拍来说需要设置降价周期
     * (paimai_info.interval)和拍卖保留价(paimai_info.reserve)。

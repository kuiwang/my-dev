...
public class TradesSoldGetRequest implements TaobaoRequest<TradesSoldGetResponse> {
     ...
     * seller_nick, buyer_nick, title, type, created, tid,
     * seller_rate,seller_can_rate, buyer_rate,can_rate, status, payment,
     ...
     * receiver_mobile,
     * receiver_phone,seller_flag,alipay_id,alipay_no,is_lgtype
     * ,is_force_wlb,is_brand_sale,buyer_area,has_buyer_message,
     * credit_card_fee, lg_aging_type, lg_aging,
     * step_trade_status,step_paid_fee
     * ,mark_desc,has_yfx,yfx_fee,yfx_id,yfx_type
     * ,trade_source,send_time,is_daixiao
     * ,is_wt,is_part_consign,zero_purchase <br>
     ...
     * orders.seller_type,
     * orders.end_time,orders.order_from,orders.consign_time
     * ,orders.shipping_type
     * ,orders.logistics_company,orders.invoice_no,orders.is_daixiao<br>
     ...
     * 可选值： RATE_UNBUYER(买家未评) RATE_UNSELLER(卖家未评)
     * RATE_BUYER_UNSELLER(买家已评，卖家未评) RATE_UNBUYER_SELLER(买家未评，卖家已评)
     ...
     * auto_delivery(自动发货) ec(直冲) cod(货到付款) game_equipment(游戏装备)
     * shopex_trade(ShopEX交易) netcn_trade(万网交易) external_trade(统一外部交易)
     * instant_trade (即时到账) b2c_cod(大商家货到付款) hotel_trade(酒店类型交易)
     * super_market_trade(商超交易) super_market_cod_trade(商超货到付款交易)
     * taohua(淘花网交易类型） waimai(外卖交易类型） o2o_offlinetrade（O2O交易）
     * nopaid（即时到帐/趣味猜交易类型） step (万人团) eticket(电子凭证)
